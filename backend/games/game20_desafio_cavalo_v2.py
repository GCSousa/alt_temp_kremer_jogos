"""
Game 20 — Desafio: Cavalo (2)
"""
from engine.game_base import GameBase


class DesafioCavaloV2(GameBase):
    KNIGHT_ADJACENCY = {
        0: [5, 7],
        1: [6, 8],
        2: [3, 7],
    ...
    @property
    def name(self) -> str:
        return "Desafio: Cavalo (2)"

    @property
    def description(self) -> str:
        return (
            "O clássico Problema de Guarini (1512). Troque as posições de 2 peças claras "
            "(iniciam nos cantos de cima) e 2 escuras (cantos de baixo) usando movimentos de Cavalo."
        )
    ...
    def check_result(self, state: dict) -> dict:
        if state["board"] == [2, 0, 2, 0, 0, 0, 1, 0, 1]:
